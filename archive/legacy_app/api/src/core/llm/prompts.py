...
**日本語・単位・場面の自然さ（厳守。日本の中学校のテスト/入試として不自然な設定は禁止）**:
- **通貨は「円」のみ**。「元」「ドル」「$」「ウォン」「ユーロ」等の外国通貨は使わない（"元" は「元の値段」等の意味でのみ可、通貨単位としては不可）。
- **離散量（人数・個数・枚数・回数・冊数など数えられるもの）には分数・小数を使わない**。中間表現データの答えや値が分数・小数になる場合、その量を「人数」「個数」等の数えられる対象に設定してはいけない。代わりに**連続量（長さ・面積・体積・重さ・時間・速さ・道のり・割合・確率・金額・座標・関数の値）**として場面を作る。
  - 悪い例: 「来園者数 $x$ 人」で答えが $\\frac{{1}}{{2}}$ → 半人は存在しない（不自然）。
  - 良い例: 「ばねに $x$ kg の重りを吊るすと…」「$x$ 秒後の高さは…」など分数でも自然な量にする。
- **単位は日本の標準**（長さ cm/m/km、重さ g/kg、時間 秒/分/時間、角度 度、金額 円 等）。
- **場面は日本の中学生に馴染むもの**にし、負の値が不自然な量（個数・長さ・人数）を負にしない。

**翻訳の忠実性（捏造の禁止・厳守。中間表現データが唯一の正）**:
- `sub_questions` の `prompt_hint` と `answer` が「何を問い・何が答えか」の唯一の根拠。これに**存在しない数値・事象・条件・登場人物・シナリオを創作しない**。
- `logic_steps` の `narration_hint` に定理や場面の枠組み（例:「平行線と線分の比の定理より $a:b=c:x$」「相似比 $a:b$」「中点連結定理」「二等辺三角形で底角が…」）がある場合、**その枠組みをそのまま問題文の骨組みに使う**（別の設定に置き換えない）。
- **小問は与えられた `sub_questions` の数だけ**作る。勝手に小問を増減しない。1 問なら 1 問だけ。
- 確率・場合の数では、標本空間（さいころ 6 通り・硬貨 表裏・玉/カードの内訳など）と事象条件を `narration_hint` に従って**正確に写す**。データに無い「和が $k$」等の条件を勝手に作らない。
- `answer` が図形の合同/相似の式（例 `△ABC ≡ △DEF`）なら、問いは「対応する頂点の順に記号で表しなさい」型にし、図に無い辺の長さ等を問わない。
