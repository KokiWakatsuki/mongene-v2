"""規則カタログ（docs/proof_engine_design_2026-08-08.md §5）。
...
def _pairs_of_triangles(points: list[Point]) -> Iterable[tuple[tuple, tuple]]:
    """異なる2つの三角形の組（対応つき）。同じ頂点集合の別の対応も含む。"""
    tris = _triangles(points)
...
def _apply_sss(points: list[Point], facts: frozenset[Fact]) -> Iterable[Derivation]:
    for t1, t2 in _pairs_of_triangles(points):
        premises = []
        for i in range(3):
            j = (i + 1) % 3
