...
# 「元」= 未知数の個数を数える構造語（例「2元1次方程式」の "2元"）。「次」と同様に、
# 答えの数値（例 傾き 2）と偶然一致して漏洩誤検出になるため除外する（graph_table「かく」で発覚）。
# 「量」「数」「区間」「種類」「段」「象限」「項」も、上と同じ**数える語**である
# （「比例する2量」「2数の大小」「1区間の動点」）。ヒントを概念の名前から作る
# ようにしたとき、これらが無いために 232 小問でヒントが作れなかった。
# ★「通り」は入れない——「起こりうる場合は全部で31通り」のように**答えそのもの**が
# その形で出る単元がある。
_COUNTER_EXPR_RE = re.compile(
    r"\d+\s*(?:つ|点|個|本|回|番|枚|人|組|冊|台|匹|問|桁|倍|割|面|辺|角|次|元|乗|号|階|歳|才"
    r"|量|数|区間|種類|種|段|象限|項|式|文字|直線|平面|変数|位)"
)
