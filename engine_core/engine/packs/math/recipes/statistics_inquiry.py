...
# 記録の種目と、中学生の記録として実際にありうる値の幅。
# 「立ち幅とびの助走時間」は立ち幅とびに助走が無いので外した。
# **種目名の末尾に「の記録」を入れない**（本文が「{種目}の記録が次のように」と
# 続けるので「…の記録の記録が」と重複していた）。
# 距離は算用数字で揃える（「二十メートル走」だけ漢数字になっていた）。
#
# **単位を「十分の一秒」にしない。** 数を小さく収めるために作った単位で、
# 実物の問題集にはこの単位が無い（「50m走のタイム（十分の一秒）: 82」と出ていた）。
# 秒・分のまま整数で書ける種目に入れかえる。どれも「小さいほうがよい」量である
# （このセルは smaller_is_better を前提にしている）。
