...
# ---------------------------------------------------------------------------
# **どの格子点かを指定する。**「一つ読み取り」と言いながら答えを1つしか持って
# いなかったので、正解のはずの (2, 7) を書いた生徒が不正解になっていた
# （EVALUATION D-9）。x座標だけを与えても、y は図から読むしかない。
PROPG_READ_LATTICE_V1 = (
    "座標平面上に、原点を通る直線のグラフがある。このグラフが通る格子点"
    "（x座標もy座標も整数である点）のうち、x座標が{{ given.x_target }}である点の"
    "座標を答えよ。"
)
...
# ---------------------------------------------------------------------------
# g1_l36.graph_table Lv3: 比例と反比例のグラフを1つの座標平面で比べ交点を読む
#
# ★もとは料金プランの比較（A社・B社）だった。中2「1次関数」の内容だったので
# 中1 の比例・反比例に差し替えてある（recipe 側の注記を見よ）。
# ---------------------------------------------------------------------------
PROPG_TWO_PLANS_GRAPH_V1 = (
    "{{ given.condition }}を、同じ座標平面上にグラフでかき、"
    "二つのグラフの交点の座標（両方の条件を同時に満たす縦と横）を読み取れ。"
)
