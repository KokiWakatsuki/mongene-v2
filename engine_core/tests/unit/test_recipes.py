...
# `Path("engine/curriculum/...")` はリポジトリの根から実行することが暗黙の前提で、
# ディレクトリを移した瞬間に family が0件になり、ここのテストが全部落ちる（実際に落ちた）。
FAMILIES_DIR = (
    Path(engine.__file__).resolve().parent / "curriculum" / "math" / "families"
)
