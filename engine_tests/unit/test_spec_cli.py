...
def test_preview_default_out_path(tmp_path: Path, monkeypatch) -> None:
    # families-dir は絶対パスで明示しつつ、出力の既定パス（cwd 相対）だけを検証する。
    # cwd を tmp_path に変えても families を解決できるよう --families-dir を渡す。
    families_dir = Path.cwd() / "engine" / "curriculum" / "math" / "families"
    monkeypatch.chdir(tmp_path)
    exit_code = main(
        ["preview", "math.g2_l25.find_value", "--seeds", "1", "--families-dir", str(families_dir)]
    )
    assert exit_code == 0
