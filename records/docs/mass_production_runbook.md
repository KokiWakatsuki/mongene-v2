...
| 役割の型で場面が乗る棚 | ⚠️ **5つ・27場面**（1元1次6／2次5／比例と度数4／連立7／文字の式5）。棚に乗っているのは **22 / 630 セル（3.5%）**。1元1次は8関係のうち2つだけが棚（残り6関係は旧経路） |
...
★次に棚へ乗せられるのは **1元1次の残り6関係**（`catch_up`・`continued_ratio`・
`proportion_pair`・`round_trip`・`seat_shortage`・`surplus_shortage`）。
同じ module の中なので、`_RELATION_ROLES` に足して場面を棚へ移すだけで済む。
