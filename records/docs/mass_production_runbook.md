...
| 役割の型で場面が乗る棚 | ⚠️ **2つ**（`scenes.py` 6場面＝1元1次／`word_problem_quadratic.SCENE_SHELF` 4場面＝2次）。残り3モジュール（連立・比例と度数・文字の式）は1対1のまま |
...
つまり**いま量産テストを回せるのは一次方程式と2次方程式の文章題**。
残り3モジュール（連立・比例と度数・文字の式）に広げるには先に棚出し
（役割の型キーへの移行）が要る。
