...
`scratchpad/bt/problems.md` に書き出す。読み手は `scratchpad/bt/answers.tsv` に
`セル<TAB>seed<TAB>立てた式` を書く。`bt_check.py` がエンジンの式と突き合わせる。

**読み手が recipe を読んでしまうと独立でなくなる。** 書き出すのは日本語だけ。

実行:
  PYTHONPATH=. .venv/bin/python scratchpad/bt_dump.py [--seeds N]
"""
...
def main() -> int:
    ...
    print(f"{len(rows)} 問を scratchpad/bt/problems.md に書き出した（{seeds} seed/セル）")
    return 0
