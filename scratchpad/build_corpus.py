...
実行: PYTHONPATH=. .venv/bin/python scratchpad/build_corpus.py
"""
...
def load_cells() -> list[tuple[str, str, int, list[set], int]]:
    ...
    **値の集合**を持ち回り、MR のどのパラメータがその集合から値を取っているかを
    ...
    """
    cells = []
    for path in sorted(glob.glob("engine/curriculum/math/families/*.yaml")):
...
def main() -> None:
    ...
    print(f"問題 {n_problems} 個 / 図 {n_figs} 枚 → scratchpad/corpus/INDEX.md")
    if shortfalls:
