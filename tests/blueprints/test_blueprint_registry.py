...
def test_angle_calculation_runs(tmp_path: Path) -> None:
    runner = _make_runner(tmp_path / "dedup.db")
    request = GenerationRequest(target_difficulty=45, problem_form="calculation", lesson_id="g2_l40")
    result = runner.run(request, _mapping_for("AngleCalculationStructure", 2, 45, "calculation"))
    assert result.middle_representation.sub_questions
