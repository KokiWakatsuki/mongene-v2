...
def test_full_coverage_meets_quality_gates() -> None:
    ...
    # 2. 100% 成功率
    assert report.successes == 184, f"{report.failures} 件失敗: {report.errors[:5]}"

    # 3. Solvability 100%（必須、CI ゲート）
    assert report.solvability_pass == 184

    # 4. Accuracy（モック前提では Solvability と同等）
    assert report.accuracy_pass == 184

    # 5. Appropriateness 100%
    assert report.appropriateness_pass == 184

    # 6. Standards Alignment 100%（1〜2 件の許容誤差を認める）
    assert report.standards_pass >= 175, f"Standards Alignment が {report.standards_pass}/184"
